...
function Carousal() {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get(`https://thankful-loafers-hare.cyclic.app/women`);
      let x = res.data;
      x.length=10;
      setGetData(x);
    } catch (err) {
  ...
  whiteSpace: 'nowrap'}}>{item.name}</h1>
            </div>
      ...
      <Link to="/womensProduct"><Button mt="30px" bg="black" color="white"_hover={{bg:"black"}}>SEE ALL <AiOutlineDoubleRight/></Button></Link>
    </div>
  );
}
